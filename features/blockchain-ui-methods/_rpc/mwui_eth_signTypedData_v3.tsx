...
function useSignTypedDataV3Pages() {
  /* Marshall SignatureRequestPage parameters for MC SignTypedData V3 */
  ...
  const onSignRequest = async () => {
    if (!payload) return;
    sigResult = await store.dispatch(UserThunks.signTypedDataV3ForUser(normalizeTypedData(message)));
  };
  const onComplete = () => {
...
const PageRender = () => {
  ...
  const signTypedDataV3Pages = useSignTypedDataV3Pages();

  const { page, resolvePage } = useController([...signTypedDataV3Pages.routes, ...genericErrorPages.routes]);
...
const mwui_eth_signTypedData_v3: RpcRouteConfig = {
  middlewares: [
    atomic(),
...
export default createFeatureModule.RPC(mwui_eth_signTypedData_v3);
